...
public class Duvarlar : Engel
{
    ...
    public Duvarlar(int baslangic_x, int baslangic_y, int yukseklik, int genislik, int id) : base(baslangic_x, baslangic_y, yukseklik, genislik, id)
    {
    ...
    public override bool IsColliding(Vector3 position, int objectSizeY, int objectSizeX, int kontrol_uzunlugu, bool tek_mi, int int_boyut)
    {
        ...
        if (position.x < -int_boyut/2+kontrol_uzunlugu || position.x >= int_boyut/2-kontrol_uzunlugu || position.y < -int_boyut/2+1 || position.y >= int_boyut/2)
        {
        ...
        */
        ...
        position.y -= 1; 
        //Debug.Log("ilk kontrol****" + position.x + " " + position.y);
        for (int y = 0; y < objectSizeY; y++)
        {
            for (int x = 0; x < objectSizeX; x++)
            {
                //Debug.Log("dizi kontrol*" + ((int)position.y + y) + " " + ((int)position.x + x + " " + grid.GetGrid()[(int)position.y + y, (int)position.x + x]));
     ...
     public override GameObject SpawnObject(Vector3 spawnPosition, GameObject prefab, int objectSizeY, int objectSizeX, bool tek_mi, int kontrol_uzunlugu, int int_boyut)
    {
        ...
        //Debug.Log("inst" + spawnPosition.x + " " + spawnPosition.y);
    ...
    public override void  AssignObjectId( Vector3 position, int objectSizeY, int objectSizeX, int kontrol_uzunlugu,int  int_boyut)
    {
        ...
        Debug.Log("dizi yazdirma***" + position.x + " " + position.y);
        for (int y = 0; y < objectSizeY; y++)
        {
            for (int x = 0; x < objectSizeX; x++)
            {
